...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold m-0">Planifier une Session</h3>
        ...
        <form action="{{ route('admin.sessions.store') }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="formation_id" class="form-select @error('formation_id') is-invalid @enderror" required>
                        ...
                        @foreach($formations as $formation)
                            <option value="{{ $formation->id }}" {{ old('formation_id') == $formation->id ? 'selected' : '' }}>{{ $formation->titre }} ({{ $formation->duree }})</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="formateur_id" class="form-select @error('formateur_id') is-invalid @enderror" required>
                        ...
                        @foreach($formateurs as $formateur)
                            <option value="{{ $formateur->id }}" {{ old('formateur_id') == $formateur->id ? 'selected' : '' }}>{{ $formateur->nom }} {{ $formateur->prenom }} - Spécialité : {{ $formateur->specialite }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" name="matiere" class="form-control @error('matiere') is-invalid @enderror" value="{{ old('matiere') }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="date_fin" class="form-control @error('date_fin') is-invalid @enderror" value="{{ old('date_fin', date('Y-m-d')) }}" required>
                    @error('date_fin') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="col-md-4">
                    <label class="form-label form-label-required">Heure de début</label>
                    <input type="time" name="heure_debut" class="form-control @error('heure_debut') is-invalid @enderror" value="{{ old('heure_debut', '09:00') }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <input type="time" name="heure_fin" class="form-control @error('heure_fin') is-invalid @enderror" value="{{ old('heure_fin', '11:00') }}" required>
                    ...
                    <div class="d-flex flex-wrap gap-3 p-3 border rounded @error('jours') border-danger @enderror">
                        @foreach(['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'] as $jour)
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="jours[]" value="{{ $jour }}" id="jour_{{ $jour }}" {{ is_array(old('jours')) && in_array($jour, old('jours')) ? 'checked' : '' }}>
            ...
            <div class="mt-4 text-end">
                <button type="submit" class="btn btn-primary"><i class="bi bi-save me-1"></i> Planifier la session</button>
            </div>
        </form>
    </div>
</div>
