...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold m-0">Modifier la Formation</h3>
        ...
        <form action="{{ route('admin.formations.update', $formation->id) }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-12">
                    ...
                    <input type="text" name="titre" class="form-control @error('titre') is-invalid @enderror" value="{{ old('titre', $formation->titre) }}" required>
                    @error('titre') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                
                <div class="col-md-4">
                    <label class="form-label form-label-required">Durée</label>
                    <input type="text" name="duree" class="form-control @error('duree') is-invalid @enderror" placeholder="Ex: 3 mois" value="{{ old('duree', $formation->duree) }}" required>
                    @error('duree') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label form-label-required">Prix (MAD)</label>
                    <input type="number" step="0.01" name="prix" class="form-control @error('prix') is-invalid @enderror" value="{{ old('prix', $formation->prix) }}" required>
                    @error('prix') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label form-label-required">Places disponibles</label>
                    <input type="number" name="places" class="form-control @error('places') is-invalid @enderror" value="{{ old('places', $formation->places) }}" required min="0">
                ...
                <div class="col-md-12">
                    ...
                    <select name="formateur_id" class="form-select @error('formateur_id') is-invalid @enderror" required>
                        ...
                        @foreach($formateurs as $formateur)
                            <option value="{{ $formateur->id }}" {{ old('formateur_id', $formation->formateur_id) == $formateur->id ? 'selected' : '' }}>{{ $formateur->nom }} {{ $formateur->prenom }} - {{ $formateur->specialite }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-12">
                    ...
                    <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="4">{{ old('description', $formation->description) }}</textarea>
